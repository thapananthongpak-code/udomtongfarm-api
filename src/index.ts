  ...
  if (SENDGRID_API_KEY_VAR) {
    try {
      await withTimeout(
        sgMail.send({
          to,
          from: { email: SENDGRID_FROM || EMAIL_USER, name: 'Udomtong Farm' },
          subject,
          html,
          ...
          headers: {
            ...
            'List-Unsubscribe': `<mailto:${SENDGRID_FROM || EMAIL_USER}?subject=unsubscribe>`,
          },
        }),
        15000,
      );
...
app.post('/api/forgot-password', authLimiter, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const emailSent = await sendEmail(
      email,
      'Reset Your Password — Udomtong Farm',
